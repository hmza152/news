...
| – | Integration, prompt iteration on real stories | 2 weeks |

## Total

- **~5 weeks** to a usable v1.
...
2. **Sensitivity filter for crime stories** — must be a separate guardrail layer, not bolted on.
3. **Lip-sync quality on Urdu phonemes** (HeyGen/D-ID) — verify on Urdu samples before committing.
